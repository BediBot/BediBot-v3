        ...
        for (let i = 0; i < quotes.length; i += MAX_QUOTES_PER_PAGE) {
            ...
            for (let j = 0; j < MAX_QUOTES_PER_PAGE; j++) {
                ...
                if (user)
                    ...
                    if (name.length > (EMBED_MAX_CHAR_LENGTH - field.length - 2))
                        name.slice(EMBED_MAX_CHAR_LENGTH - field.length - 2);
                    field += `${Formatters.inlineCode(quotes[i + j].name)}`;
                }

                // Just in case the field length is somehow too big, this will prevent an error from stopping the
