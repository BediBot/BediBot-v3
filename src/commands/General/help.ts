...
module.exports = class HelpCommand extends Command {
    ...
    async messageRun(message: Message, args: Args) {
        ...
        if (!selectedCommand || !this.store.has((selectedCommand.toLowerCase()))) {
            ...
            if (message.guild) {
                const serverReplyEmbed =
                    new BediEmbed().setTitle('Help Reply').setDescription('A list of commands has been sent to you via DM.');
            ...
            const command = this.store.get(selectedCommand.toLowerCase());

            const embed =
                new BediEmbed().setTitle(`Help Reply - ${prefix}${command.name} command`).setDescription(command.description);
