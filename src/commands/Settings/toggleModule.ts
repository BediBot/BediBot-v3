...
module.exports = class SettingsCommand extends Command {
    ...
    async messageRun(message: Message, args: Args) {
        ...
        const selectRow = new MessageActionRow().addComponents(
            new MessageSelectMenu().setCustomId('moduleSelect').setPlaceholder('Select a Module').addOptions([
                {
                    label: 'Pins',
        ...
        const buttonRow = new MessageActionRow().addComponents([
            new MessageButton().setCustomId('moduleEnable').setLabel('Enable').setStyle('SUCCESS'),
            new MessageButton().setCustomId('moduleDisable').setLabel('Disable').setStyle('DANGER'),
        ]);
        ...
        buttonCollector.on('collect', async interaction => {
            ...
            if (!module) {
                const embed = new BediEmbed().setTitle('Toggle Modules Reply').setDescription('Please select a module first');
            ...
            const embed = new BediEmbed().setTitle('Toggle Modules Reply');

            if (toggleValue)
                embed.setColor(colors.SUCCESS).setDescription(`The ${Formatters.inlineCode(module)} module has been enabled`);
            else
                embed.setColor('RED').setDescription(`The ${Formatters.inlineCode(module)} module has been disabled`);
